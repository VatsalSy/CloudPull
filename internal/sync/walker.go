...
type WalkerConfig struct {
  ...
  // Include patterns (regex) - if set, only matching folders are processed
  IncludePatterns []string
  
  // Exclude patterns (regex) - matching folders are skipped
  ExcludePatterns []string
  
  // Whether to follow Google Drive shortcuts
  FollowShortcuts bool
  
  // Number of concurrent folder scanners
  Concurrency int
  
  // Channel buffer size for streaming results
  ...
  for i := 0; i < workers; i++ {
    ...
    go func(workerID int) {
      ...
      for task := range queue {
        ...
        if err == nil && (fw.config.MaxDepth == 0 || task.depth < fw.config.MaxDepth) {
